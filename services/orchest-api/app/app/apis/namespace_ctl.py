...
class ListPods(Resource):
    ...
    def get(self):
        ...
        query_string = []
        if project_uuid:
            query_string.append(f"project_uuid={project_uuid}")

        if pipeline_uuid:
            query_string.append(f"pipeline_uuid={pipeline_uuid}")
        response = k8s_core_api.list_pod_for_all_namespaces_with_http_info(
            label_selector=",".join(query_string)
        )
        api_response = []
        for item in response[0].items:
            api_response.append(
                {
                    "status": self._prepare_status(item),
