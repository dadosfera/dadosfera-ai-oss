...
def write_environment_dockerfile(
    ...
    if os.environ.get('CLOUD_ENVIRONMENT','azure') in _utils.NON_AWS_CLOUD_ENVIRONMENTS:
        _logger.info('Detected cloud environment different than AWS')
        if _config.CONTAINER_RUNTIME == 'docker':
            statements.append(
                "RUN --mount=type=secret,id=aws_secrets "
                "--mount=type=secret,id=aws_config "
                "mkdir $HOME/.aws "
                "&& sudo cat /run/secrets/aws_secrets > $HOME/.aws/credentials "
                "&& sudo cat /run/secrets/aws_config > $HOME/.aws/config "
            )
        elif _config.CONTAINER_RUNTIME == 'containerd':
            statements.append(f"ARG AWS_ACCESS_KEY_ID")
            statements.append(f"ARG AWS_SECRET_ACCESS_KEY")
            statements.append(f"ARG AWS_SESSION_TOKEN")
...
def prepare_build_context(task_uuid, project_uuid, environment_uuid, project_path):
    ...
    try:
        ...
        _utils.copytree(userdir_project_path, snapshot_path, use_gitignore=True)
    except OSError as e:
...
def build_environment_image_task(
    ...
    with requests.sessions.Session() as session:
        ...
        try:
            ...
            # Cleanup.
            _utils.rmtree(build_context["snapshot_path"])
