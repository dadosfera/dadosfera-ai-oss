...
def _should_constrain_to_nodes_with_image(
    ...
    """
    Determines whether a pod should be constrained to nodes that already have the required image.

    This logic has been updated to decouple the application from node-specific constraints.

    Previous Behavior:
    - For the "interactive" scope, pods were always constrained to nodes that already had the image
      to prioritize faster availability for users.
    - For non-interactive scopes, pods were constrained to specific nodes only if the image was
      not available in the registry, as pulling the image would not be possible.

    While this behavior was designed to optimize performance and ensure image availability,
    it introduced unnecessary coupling to specific nodes. This caused scheduling issues when
    nodes were recreated, particularly in single-node pools where node names frequently change,
    resulting in errors such as:
        "nodeinfo not found for node name 'aks-default-23462846-vmss000000'".

    Updated Behavior:
    - The logic now avoids constraining pods to specific nodes unless the image is unavailable in
      the registry. This ensures that the application can dynamically schedule workloads without
      relying on specific node names.
    - For clusters with a single node pool, this change improves reliability by eliminating errors
      caused by node recreation.

    Rationale for the Change:
    - Kubernetes is designed to pull images dynamically from the registry as needed, making node
      constraints unnecessary when images are available.
    - Decoupling from specific nodes aligns with Kubernetes best practices, making the application
      more robust and flexible in dynamic environments.

    Args:
        scope (str): The scope of the workload, e.g., "interactive" or "non-interactive".
        image_is_in_registry (bool): Whether the image is available in the registry.

    Returns:
        bool: True if the pod should be constrained to nodes with the image; False otherwise.
    """
    return not image_is_in_registry
...
def modify_jupyter_eg_scheduling_behaviour(manifest: Dict[str, Any]) -> None:
    _modify_deployment_pod_scheduling_behaviour("interactive", manifest, _Plane.WORKER)
