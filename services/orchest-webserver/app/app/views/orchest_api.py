...
def register_orchest_api_views(app, db):
    ...
    @app.route(
        "/catch/api-proxy/api/environments",
        methods=["GET"]
    )
    def catch_api_environments():
        resp = requests.get(
            "http://"
            + app.config["ORCHEST_API_ADDRESS"]
            + "/api/environments",
    ...
    @app.route("/catch/api-proxy/api/ctl/pods", methods=["GET"])
    def api_proxy_ctl_list_pods():
        params = {
        ...
        try:
            resp = requests.get(
                f"http://{app.config['ORCHEST_API_ADDRESS']}/api/ctl/pods",
                params=params
            )
