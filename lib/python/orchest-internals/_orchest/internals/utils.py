...
NON_AWS_CLOUD_ENVIRONMENTS = ['azure','mgc']

def get_temp_aws_volumes():
    if os.environ.get('CLOUD_ENVIRONMENT',"azure") in NON_AWS_CLOUD_ENVIRONMENTS:
        return [{
                "name": "aws-credentials",
                "secret": {
                    "secretName": "aws-customer-iam-role"
                }
        }]

    return []

def get_temp_aws_volume_mounts():
    if os.environ.get('CLOUD_ENVIRONMENT',"azure") in NON_AWS_CLOUD_ENVIRONMENTS:
        return [
        {
            "name": "aws-credentials",
...
def get_aws_env_vars():
    if os.environ.get('CLOUD_ENVIRONMENT',"azure") in NON_AWS_CLOUD_ENVIRONMENTS:
        return [
            ...
            {"name": "AWS_SHARED_CREDENTIALS_FILE","value":"/tmp/.aws/credentials"}
        ]
...
def get_step_and_kernel_volumes_and_volume_mounts(
    ...
    volume_mounts = volume_mounts + get_temp_aws_volume_mounts()
    volumes = volumes + get_temp_aws_volumes()
